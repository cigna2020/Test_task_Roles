...
const phrases = text.split('\n');   // делаем из текста массив, делим переносом строки
let result = '';                    // для текста

roles.forEach(role => {
    let temp = `\n${role}: \n`;    // начальное значение при запуске forEach

    phrases.forEach((line, num) => { // line - каждая отдельная фраза, num - нумерация строки
        // if (line.includes(role)) {    // в этом случае имена, которые внутри самого текста будут мешать - баг
        if (line.indexOf(role) >= 0 && line.indexOf(role) < role.length) {  // indexOf возвращает позицию (начало строки = 0) с которой началось совпадение
            const content = line.slice(role.length + 1); // обрезаем имя героя в начале строки
            temp += `${num + 1}) ${content}\n`;
